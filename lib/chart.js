...
var FunnelChart = function FunnelChart(_ref) {
  var width = _ref.width,
    ...
    right: 10
      ...
      hoverShift = _ref$hoverShift === void 0 ? 5 : _ref$hoverShift;
  ...
  var prepareData = function prepareData(data) {
    ...
    return data.map(function (item, i, a) {
      ...
      return {
        ...
        width: iw,
        color: cc,
        textColor: new color$1(cc).luminosity() > 0.5 ? '#000000' : '#FFFFFF',
        item: item
      };
    });
  };
  ...
  var topY = function topY(item) {
    return item.y + (hover === item.item.name ? hoverShift : 0);
  };
  ...
  var renderLabel = function renderLabel(item, i) {
    if (typeof label === 'boolean' && label) {
      ...
      return React__default.createElement("text", {
        ...
        y: topY(item) + item.height / 2 + item.width * ry,
        fill: item.textColor,
        stroke: 'none',
        strokeWidth: 0,
        fontWeight: 900,
  ...
  var renderItem = function renderItem(item, i, a) {
    var d = [item.x, topY(item), item.x + item.width, topY(item), Math.floor((w + bottomW) / 2), item.height - gap + topY(item), Math.floor((w - bottomW) / 2), item.height - gap + topY(item)];
